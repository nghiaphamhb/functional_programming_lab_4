...
let example_bot =
  chatbot "start" [
    ...
    state "main" [
      ...
      rule (Exact "инфо") [
        say "Я учебный чат-бот на OCaml. Логика описана через eDSL (правила и состояния) и выполняется интерпретатором. Интеграция: Telegram Bot API.";
      ];
      rule (Exact "автор") [
        say "Автор: Фам Данг Чунг Нгиа - студент 3 курса ИТМО. Лабораторная работа №4 (eDSL для чат-бота).";
      ];
      rule (Exact "пока") [
        say "Пока! Возвращайся :)";
        goto "start";
      ];
      rule Any [
